    ...
    $('#rombel').change(function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          html += '</table>';
          //html += '<button class="form-control btn-warning" name="tl" id="tl" onClick="tl(' + data[i].nis + ');">Terlambatt</button>';
          $('#tampil').html(html);
        }
      });
    });
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        for (i = 0; i < data.length; i++) {
          var no = i + 1;
          html += '<tr><td>' + no + '</td><td>' + data[i].nis + '</td><td>' + data[i].nama_siswa + '</td>';
          html += '<td><button class="form-control btn-warning" name="tl" id="tl" onClick="tl(' + data[i].nis + ');">Terlamba</button></td></tr>';
        }
        html += '</table>';
        $('#tampil').html(html);
      }
