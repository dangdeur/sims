...
class Cetak extends Pbm
{
	...
	public function agenda_tendik()
	{
		...
		if ($this->request->is('post')) {
			...
			$tahun = $this->request->getVar('tahun');
			$data['staf'] = $stafmodel->where('kode_staf', $data['kode_pengguna'])->first();
			$agendamodel = new AgendaTendikModel;

			$agendamodel->where('kode_staf', $data['kode_pengguna']);
			$agendamodel->like('kode_agendatendik', $bulan . date("Y"));
			$agendamodel->orderBy('tanggal', 'ASC');
			$data['agenda'] = $agendamodel->findAll();
			$data['qr'] = $this->qr();
			$data['bulan'] = BULAN[$this->request->getVar('bulan')] . ' ' . date("o");
			...
			//d($data);
			$html = view('cetakagendatendik', $data);

			$pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
			$pdf->setCreator(PDF_CREATOR);
			$pdf->setAuthor('Endang Suhendar');
			$pdf->setTitle('Agenda Tendik');
			$pdf->setSubject('SMKN 2 Pandeglang');
			// $pdf->setKeywords('TCPDF, PDF, example, test, guide');
			$pdf->setHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING);
			// set header and footer fonts
			...
			$pdf->writeHTML($html, true, false, true, false, '');
			$this->response->setContentType('application/pdf');
			$pdf->Output('Agenda Tendik ' . $data['nama_lengkap'] . ' ' . $bulan . '-' . $tahun . '.pdf', 'D');
		} else {
			return view('header')
				. view('menu', $data)
				. view('form_cetaktendik')
				//  .view('paginasi')
				. view('footer');
		}
	}
