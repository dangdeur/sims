...
class AgendaGuru extends Pbm {
    ...
    public function hapuspresensi( $absensi, $nis, $id_agendaguru ) {
        ...
        //dd( $data_absensi );
        for ( $h = 0; $h<count( $data_absensi[ $absensi ] );
        $h++ ) {
        ...
        d( $data_absensi );
        if ( empty( $data_absensi[ $absensi ] ) ) {
            unset( $data_absensi[ $absensi ] );
            $this->update_absen( $data_absensi=NULL, $id_agendaguru );
        }
        d( $data_absensi );
    ...
    public function update_absen( $absen, $id ) {
        ...
        if ($absen==NULL)
        {
          ...
          d( $dataabsen );
          if ( !empty( $dataabsen ) ||  $dataabsen != '[]') {
              //$data[ 'update' ][ 'absensi' ] = $dataabsen;
              $agenda->set( 'absensi', $dataabsen );
          } 
        }
        $agenda->where( 'id_agendaguru', $id );
        $agenda->update();
